  ...
  IF w_s_cal[0] NE -1 AND w_s_tgt[0] NE -1 THEN BEGIN
    ...
    FOR i=0, N_ELEMENTS(match_substs)-1 DO BEGIN ; substances loop
      ...
      IF estimate_cal THEN BEGIN
        w_fin = WHERE(FINITE(tgt_mrs[*,i]) EQ 1, n_fin)
        w_fin_rR = WHERE(FINITE(samples_rR) EQ 1, n_fin)
...
; +++ MOD       
;      IF match_substs[0] EQ 'SF6' THEN fct_dgr=2
; +++
      ...
      IF status EQ 0 THEN BEGIN ; polyfit successful completed?
...
; +++ MOD 
;          IF calMR_inTGTrange EQ '! NO !' THEN cal_est_sig=cal_est_sig*2D
;          saveplot=0
;          show_plots=0
; +++
