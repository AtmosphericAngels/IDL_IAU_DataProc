...
PRO dp_plot_mrs_prc, sel_exp, sel_subst, SHOW_PRC=show_prc, SHOW_CAL_PRC=show_cal_prc, SHOW_MRS=show_mrs, EVAL_MODE=eval_mode
 ...
 COMMON dp_data
  ...
  w_cal_end = ((dp_expcfg[sel_exp]).sequence.ix_end_calblock)[WHERE(((dp_expcfg[sel_exp]).sequence.ix_end_calblock) NE -1)]
  ...
  IF show_prc THEN BEGIN
      ...
      IF show_cal_prc THEN BEGIN
          CASE eval_mode OF
              ...
              1: prc = (((dp_chrom[sel_exp]).subst[sel_subst].rres.rsp_height.cal_block_rsd)[w_cal_end])*100D
          ENDCASE
          x = INDGEN(N_ELEMENTS(prc)) + 1
          sam_names = STRING(x)
          FOR i=0, N_ELEMENTS(w_cal_end)-1 DO sam_names[i] = STRING(9b) + sam_names[i] + STRING(9b)
          n_sam = N_ELEMENTS(w_cal_end)
          t_suffix = 'calibrations'
          c = 'r'
      ENDIF ELSE BEGIN
          CASE eval_mode OF
      ...
      IF n_fin GT 0 THEN BEGIN
        ...
        p_prc = plot($
                     ...
                     XTICKVALUES=x, $
                     TITLE=subst_name+': block precision, '+t_suffix, $
                     YTITLE='PRC [%]', $
                     LINESTYLE=6, $
                     SYMBOL='D', $
                     SYM_FILLED=1, $
